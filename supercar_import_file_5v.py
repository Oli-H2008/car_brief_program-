...
def secondary_inputs():
    x = 0
    car_type = ["Lamborghini_Gallardo", "Lamborghini_Huracan, Ferrari F40, Porsche Boxster, Audi A5, BMW i8, Lotus Elise"]
    car_cost = [59, 59, 49, 39, 39, 39, 30]
    while True:
        try:
            cars_amount = int(input("\nHow many cars do you want to test? (max of 5): \n"))
        except ValueError:
        ...
        while True:
            print("\n please choose from one of these cars (costs included): ")
            car_ovr = car_type[x] + str(car_cost[x])
            for i in car_ovr:
                print(i)
                x += 1
